...
#' @import Biobase
#' @export
SCDC_ENSEMBLE <- function(bulk.eset, sc.eset.list = NULL, sc.basis.list, ct.varname, sample,
  ...
  if (!is.null(prop.input)){
    ...
    prop.list <- lapply(1:length(sc.eset.list), function(zz){
      if (length(unique(sc.eset.list[[zz]]@phenoData@data[,sample])) > 1){
        SCDC_prop(bulk.eset = bulk.eset, sc.eset = sc.eset.list[[zz]], sc.basis = sc.basis.list[[zz]], 
        ...
        SCDC_prop_ONE(bulk.eset = bulk.eset, sc.eset = sc.eset.list[[zz]], sc.basis = sc.basis.list[[zz]], 
                      ct.varname = ct.varname, sample = sample, truep = truep,
                      ct.sub = ct.sub, iter.max = iter.max, nu = nu, epsilon = epsilon, weight.basis = weight.basis,
